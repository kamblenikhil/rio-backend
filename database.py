...
class Database:
    ...
    def getRenterDetails(self, product_id):
        self.cur.execute(
            "SELECT UserID FROM user_product WHERE ProductID = %s and Method = 2", (product_id))
        self.con.commit()
        res_det = self.cur.fetchall()
        user_id = res_det[0]['UserID']
        result = self.cur.execute(
            "SELECT UserID, FName, LName from user where UserID = %s", (user_id))
        self.con.commit()
        return result
    ...
    def getApprovedProduct(self):
        result = self.cur.execute(
            "SELECT * FROM Product WHERE Status = %s", (2))
        return result
